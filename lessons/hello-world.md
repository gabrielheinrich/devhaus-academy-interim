...
- **node**: Command Line Tool that can run javascript files (.js)
- **Typescript**: better version of Javascript, but has to be compiled using `tsc` command line tool (.ts -> .js)
- **npm**: Node package manager: Command line tool to install online registry of javascript libraries
- **yarn**: Fancy version of npm
...
- Initialize npm inside the repository by running `yarn init -y` (-y: yes to all
...
- Install the typescript compiler tsc by running `yarn global add tsc`
- Create an 'index.ts' file, and add some javascript/typescript code
- Run `tsc index.ts`. A new file `index.js` will be created, which contains the compiled code.
- Run the index.js file just as before using `node index.js`

#### Using tsconfig.json

- The typescript compiler has many options that can be set and for most projects we keep track of these options in a separate file we store in the repository under the name `tsconfig.json`. Here's an example
...
- If you now run tsc without arguments in this directory, its automatically looking for a file `src/index.ts` and will create a corresponding javascript file `build/index.js`
...
- What is npm? What is the difference between npm & yarn?
...
- Setup a index.html file with a javascript file
- Setup a javascript hello world repository.
- Setup a bare minimum hello world typescript repository
- Setup a yarn repository

[Example Repositories](https://github.com/gabrielheinrich/hello-typescript)
